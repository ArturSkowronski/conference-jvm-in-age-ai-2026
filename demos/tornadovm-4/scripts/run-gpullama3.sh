...
# GPULlama3.java Runner for TornadoVM 4.0
# GPU-accelerated LLM inference using TornadoVM 4.0 with Metal/OpenCL/PTX backends
...
Notes:
  - Requires TornadoVM 4.0 (JDK 21+) installed.
  - Set TORNADOVM_HOME to your TornadoVM 4.0 installation directory.
  - Model must be in FP16 format (Q4_K_M and other quantized formats not supported).
  - The first run clones + builds https://github.com/beehive-lab/GPULlama3.java into demos/tornadovm-4/build/.
...
Examples:
  ...
  export TORNADOVM_HOME=./demos/tornadovm-4/build/tornadovm-sdk/tornadovm-4.0.0-jdk21-metal
...
# Auto-setup TornadoVM 4.0 if not set
if [[ -z "${TORNADOVM_HOME:-}" ]]; then
  echo "TORNADOVM_HOME not set. Attempting auto-setup via run-tornado.sh..."
  # Source the tornado script's setup functions
  SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"

  # Run the setup portion only
  export TORNADOVM_VERSION="${TORNADOVM_VERSION:-4.0.0}"
  export TORNADOVM_JDK="${TORNADOVM_JDK:-jdk21}"
  ...
  detect_backend() {
    ...
    case "$platform" in
      mac-aarch64) echo "metal" ;;
      mac-amd64)   echo "opencl" ;;
      linux-*)     echo "opencl" ;;
    esac
  }
  ...
  if [[ -d "$sdk_path" ]]; then
    ...
    echo "TornadoVM 4.0 SDK not found. Run ./scripts/run-tornado.sh first to download it." >&2
