...
public final class VectorAddTornado {
    ...
    public static void main(String[] args) throws TornadoExecutionPlanException {
        ...
        try (TornadoExecutionPlan executor = new TornadoExecutionPlan(immutableTaskGraph)) {
            ...
            System.out.printf("TornadoVM 4.0: size=%d, best=%.3f ms, throughput=%.2f GB/s%n", parsed.size, bestMs, gbPerSec);
        ...
        if (parsed.verify) {
            ...
            for (int i = 0; i < heapC.length; i++) {
                if (heapC[i] != 3) {
                    throw new AssertionError("Verification error at i=" + i + " c=" + heapC[i]);
                }
            }
